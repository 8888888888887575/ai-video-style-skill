...
收到简报
  ...
  └─ 否，只有内容描述？ ──▶ 从描述中提取信号：
       ...
       是否有「成人/性感写真/NSFW/亚洲写真/JAV 风格」→ 偏 NSFW/亚洲成人（见 styles/09-nsfw-asian-亚洲人像NSFW.md）
       是否有「二创/整活/换脸/名场面复刻/鬼畜/卡点/跨IP联动/梗图/表情包/风格迁移」→ 偏 二创整活（见 styles/10-fanwork-同人二创.md，参考图锁身份）
  │
  ▼
...
|---|---|---|---|---|---|
| **PV / 动漫 MV** | MiniMax H3 | Vidu Q3 | Kling 3.0 / PixVerse v5.6 | [styles/01-pv-PV宣传片.md](styles/01-pv-PV宣传片.md) | 要卡点→需原生音频模型；锁脸用首尾帧 |
| **CG 写实 3D** | Veo 3.1 | Sora 2 | Seedance 2.0 / Kling O03 | [styles/02-cg-CG写实.md](styles/02-cg-CG写实.md) | 明确 `3D rendered not footage`；避 `melting` |
| **MG 动态图形** | **Animora/MotionVid** | Runway Gen-4 | Pika 2.0 / Wan 2.7(文字) | [styles/03-mg-MG动画.md](styles/03-mg-MG动画.md) | **勿用 Veo/Sora**；屏内文字用 Wan |
| **手绘 2D** | MiniMax H3 | Vidu Q3 | Kling 3.0 / PixVerse v5.6 | [styles/04-handdrawn-手绘动画.md](styles/04-handdrawn-手绘动画.md) | 排除 `3D render/CGI`；控 `boiling line` |
| **AI 漫剧 / 叙事** | Wan 2.7 | Seedance 2.0 | Vidu Q3 / Kling 3.0 | [styles/05-ai-drama-AI剧情短片.md](styles/05-ai-drama-AI剧情短片.md) | **跨镜头一致第一**：参考图/首尾帧/角色锁定；拆 shot list |
| **商业/电商/产品** | Wan 2.7 | Seedance 2.0 | Kling 3.0/O03 / Luma Ray 3 | [styles/06-commercial-商业广告.md](styles/06-commercial-商业广告.md) | 真实材质 `no plastic`；文字/LOGO 用 Wan 或后期；换装 I2V+首尾帧 |
| **建筑/室内渲染** | Veo 3.1 | Sora 2 / Seedance 2.0 | Wan 2.7 / Luma Ray 3 | [styles/07-architecture-建筑空间.md](styles/07-architecture-建筑空间.md) | 参考图/草图 I2V 锁结构比例；玻璃 `clear reflections` |
| **写真/人像摄影** | Veo 3.1 | Kling 3.0 / Seedance 2.0 | MiniMax H3(厚涂) / Vidu Q3 | [styles/08-portrait-人物肖像.md](styles/08-portrait-人物肖像.md) | 锁脸参考图+首尾帧；厚涂必须排 `photorealistic` |
| **NSFW / 亚洲成人** | Veo 3.1 / Sora 2 | Kling 3.0 / Seedance 2.0 | MiniMax H3 / Vidu Q3 / PixVerse | [styles/09-nsfw-asian-亚洲人像NSFW.md](styles/09-nsfw-asian-亚洲人像NSFW.md) | I2V+首尾帧锁人物；运行于闭源平台（平台自带审查）；逐词模板见上游仓库 |
| **二创整活 / 换脸 / 复刻** | Wan 2.7(I2V+首尾帧) | Kling 3.0 / Seedance 2.0 | MiniMax H3(鬼畜) / PixVerse(loop) / Vidu Q3 | [styles/10-fanwork-同人二创.md](styles/10-fanwork-同人二创.md) | **身份一致第一**：换脸/复刻/联动/迁移必用参考图+首尾帧锁身份；鬼畜 loop 首尾帧一致 |
...
### Q9：是建筑 / 室内 / 写真 / 人像？
- 建筑渲染 → Veo/Sora/Seedance/Kling（写实）+ Wan/Luma（草图 I2V 锁结构，见 styles/07-architecture-建筑空间.md）。
- 写真人像 → Veo/Kling/Seedance（写实，见 styles/08-portrait-人物肖像.md）；厚涂/插画走 MiniMax H3/Vidu/Wan，且必须排 `photorealistic`。

### Q10：是 NSFW / 亚洲成人 / 性感写真？
- **模块对齐**：直接走 `styles/09-nsfw-asian-亚洲人像NSFW.md`，其结构严格对齐 [`ShuaiHui/nsfw-prompt-templates-asian`](https://github.com/ShuaiHui/nsfw-prompt-templates-asian) 的 15 模块分类法（场景/景别/裸露液体/服装/光影/姿势/表情/胶片/妆容/发型/瑕疵/纹身/道具/人格卡）。
...
- **提示词范式（H3 专属三段式）**：`参考素材说明 + 核心创意 + 画面过程说明`；台词长短对齐镜头、需出现的文字/Logo 写出原文、跨 shot 的 J-cut/L-cut 明确写出即可响应。速查见 `prompt-templates.md 模板 K`；**完整 @标注分类 / 镜头拆分铁律 / 真实示例 / API 结构化格式 → `references/h3-guide-H3完整提示词指南.md`**。
- 其它强多模态/编辑模型：Vidu Q3（首尾帧+多输入）、Wan 2.7（首尾帧+思考模式）、Kling 3.0/O03（人体运动+I2V）。

### Q15：Seedance 2.0 还是 MiniMax H3？（编辑/延长/一镜到底 vs 高清多模态融合）
- **选 Seedance 2.0（即梦/字节）当**：
  - 要**改已有视频**——角色替换（`将@视频1人物换成@图片N`）、剧情颠覆（`颠覆@视频1剧情`）、**视频延长**（`将@视频1延长Ns`）、定向改片段。
  - 要做**一镜到底 / 音乐卡点 / 强运镜复刻**（上传参考视频即可复刻运镜与动作）。
  - 不追求超高分辨率（Seedance 上限约 **720p**，低于 H3 的 1440p）。
  - ⚠️ 注意**合规红线：禁上传写实真人脸素材**（图片/视频均拦截），人脸需求改用风格化/非写实或换 H3。
- **选 MiniMax H3（海螺/MiniMax）当**：
  - **纯文字多模态融合**、要 **1440p 高清**、要 **原生双声道音频 / 音色克隆**。
  - 需要含**写实人脸参考图**（H3 无此硬限制）。
  - 要 API 结构化字段直调（见 `h3-guide` Part B）。
- **共性**：两者都用 `@图片N/@视频N/@音频N` 自然语言标注，≤9 图/≤3 视频/混合≤12 文件、4–15s；多数 `@` 用法互通。
- 📌 写法与 15 个官方 Case → `seedance2-guide-即梦Seedance2完整指南.md`；与 H3 指南平行，按模型选读。
